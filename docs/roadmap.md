...
- **production実施結果(2026-07-10、web完了)**: `longhorn-r1`バンドル展開(#47)・
  web移行完了。移行後のLonghorn`Scheduled`使用率は0〜49%(5ノード平均約29%)。
  **重要な教訓**: productionのPVCは(dev/stagingと異なりDR復元の経緯がなく)
  `persistentVolumeReclaimPolicy`が既定の`Delete`だったため、旧PVC削除と
  **同時に旧ボリューム自体も即座に完全削除**された。直近のLonghorn定期バックアップ
  (削除の約1.5日前が最新)以降のwp-content変更分(あればメディアアップロード等)は
  復旧不能。DB側はダンプを削除直前に取得していたため無事。今回は実害の有無を
  ユーザー確認の上で許容し先に進めたが、**次回以降はreclaimPolicyがDeleteの
  サイトを移行する際、手順2実行の直前に必ず新規バックアップを取得すること**を
  ランブックに反映した([manual-storage-migration.md](manual-storage-migration.md)
  前提条件3.)。
- **3環境の移行完了、全サイトが`longhorn-r1`/`harvester`構成に移行済み**。
  30サイト到達の再試算(dev実測ベース、上記129-141行参照)は各環境で同様の
  削減効果を確認できたため確定: ノード専用ディスク+約26GiB/ノードの追加で
