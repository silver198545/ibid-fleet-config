...
- **現状**: 1サイト=1 LoadBalancer IP。IPPoolは dev 20個 / staging 20個 /
  **本番11個(192.168.1.90-100)** しかなく、**本番は11サイトで頭打ち**。
...
### 6. WordPressコア/プラグインの定期更新サイクル

- **現状**: イメージはdigest固定=セキュリティパッチも意図的に止まっている。
  更新は「digest更新→チャートversion上げ→dev→staging→本番昇格」の手動フロー。
- **方針**: **月1回の定期更新日**を決めて運用に載せる(WPコアのセキュリティ
  リリースは頻繁)。プラグインも同じサイクルでバージョン一覧を見直す。
  `wp-file-manager`(過去に重大脆弱性の履歴あり)は必要性を再検討する。
- **トリガー**: 即時(運用ルールとして決めるだけ)。

### 7. 封印鍵ローテーションへの追従

- **現状**: Sealed Secretsコントローラは30日ごとに新しい鍵を自動追加する。
  ローテーション後に再バックアップしないと、新鍵で封印されたSecretが復元不能になる。
- **方針**: **月1回の鍵再バックアップ**をカレンダー化する(6.の定期更新日と同日に
  実施するのが忘れにくい)。または `--key-renew-period=0` でローテーション無効化を検討。
- **トリガー**: 即時(次回ローテーションは導入から30日後 = 2026-08上旬)。
