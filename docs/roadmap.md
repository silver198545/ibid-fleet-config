...
- **決定事項**: **毎月1日**を定期メンテナンス日とし、WPコア/プラグインの確認・更新
  (`wp-file-manager` の必要性再検討を含む)を7.の鍵再バックアップとまとめて実施する。
  手順は [manual-multi-env.md](manual-multi-env.md) 4章「定期メンテナンス日」に明記。
  初回は2026-08-01。
...
- **決定事項**: 個別にローテーション日を追跡せず、**6.と同じ毎月1日**に3環境分の
  鍵バックアップコマンドをまとめて実行する運用とした(コマンドは冪等なので
  ローテーションの有無を気にせず毎回実行してよい)。手順は
