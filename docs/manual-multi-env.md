...
- **バックアップ先URLを変更する場合も同様**: 先に上記patchで新URLを投入してから
  Gitを変更する(逆順だとFleetのバンドルが競合エラーで失敗し続ける)。
...
- **Sealed Secrets(Phase 2)**: サイトSecretをGit管理化し、クラスタ再構築・サイト作成を
  完全にGit駆動にする。導入時はコントローラ秘密鍵のバックアップ運用が必須。
