...
運用します。** Bitnami の `wordpress` Chart を使って、LoadBalancer 経由で公開する
冗長構成のWordPressを導入します。
...
`scripts/deploy-wordpress.sh`がこれを読み込んでデプロイします。既存Secretの名前など、
サイト固有の値はサイト名から機械的に決まるため、**標準的な構成のサイトであれば
`wordpress-<site>/`ディレクトリをGitに置く必要はありません**（`scripts/deploy-wordpress.sh`
が実行のたびにデフォルト設定をその場で生成します）。そのサイトだけチャートバージョンを
個別に固定したい、`wordpressTablePrefix`を設定したいなど、全サイト共通のデフォルトから
外れた設定をしたい場合にのみ、`wordpress-<site>/fleet.yaml`をGitにコミットして使います
（後述）。全サイト共通の設定を変える場合は`wordpress-base-values.yaml`を編集してください。
...
## 1. （任意）サイト固有の設定をGitに残したい場合はテンプレートを生成する

標準的な構成（チャートバージョンやテーブル接頭辞をデフォルトのままにする）であれば
この手順は不要で、[手順2](#2-デプロイスクリプトを実行する)に進んでかまいません。
...
`wordpress-<site>`という命名規則で統一されます。生成後、そのサイト固有の値（チャート
バージョンや`wordpressTablePrefix`等）を編集し、Gitにコミットしてください。以後、
`scripts/deploy-wordpress.sh <site>`はこのファイルの内容を優先して使用します。
...
`wordpress-web/fleet.yaml`があればその`helm.chart`/`helm.version`/`helm.values`
（このサイト固有の差分。無ければ標準のデフォルト値をその場で使用）を読み取り、
...
namespaceを削除してください（データを残したい場合は、PVCに
`helm.sh/resource-policy: keep`を付けてからuninstallしてください）。
`wordpress-<site>/fleet.yaml`をコミットしている場合はそのディレクトリもGitから削除します。
サイトを追加・削除するたびに、[README.md](../README.md)の構成一覧を更新してください。
...
- WordPress の Chart バージョンは、`wordpress-<site>/fleet.yaml`が無いサイトでは
  `scripts/deploy-wordpress.sh`内の`DEFAULT_CHART_VERSION`が使われます。そのサイトだけ
  個別のバージョンにしたい場合は`wordpress-<site>/fleet.yaml`の`helm.version`で固定して
  ください。
