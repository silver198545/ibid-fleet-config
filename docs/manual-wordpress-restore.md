...
[wordpress/fleet.yaml](../wordpress/fleet.yaml) で導入済みのWordPress（[manual-wordpress.md](manual-wordpress.md)参照）に、
既存の（別環境の）WordPressサイトからバックアップしたデータを流し込む手順です。
...
DBとwp-contentを丸ごと上書きするため、`wordpress`名前空間の各PVC
...
kubectl -n wordpress exec <wordpress-pod> -- grep table_prefix /bitnami/wordpress/wp-config.php
```

Bitnamiのデフォルトは `wp_` です。旧サイトの接頭辞と異なる場合、
`wordpress/fleet.yaml` の `helm.values` に `wordpressTablePrefix` を設定します。
...
# スケールダウン
kubectl -n wordpress scale deploy base-infra-wordpress --replicas=0
kubectl -n wordpress scale statefulset base-infra-wordpress-mariadb --replicas=0

# Podが無くなったことを確認してからPVCを削除
kubectl -n wordpress get pods
kubectl -n wordpress delete pvc base-infra-wordpress
kubectl -n wordpress delete pvc data-base-infra-wordpress-mariadb-0

# スケールを戻す(mariadbを先に起動してからwordpressを起動)
kubectl -n wordpress scale statefulset base-infra-wordpress-mariadb --replicas=1
kubectl -n wordpress scale deploy base-infra-wordpress --replicas=2
```
...
Podがスケジュールされない場合は、Fleetの再同期を待つか、Rancher UIの
Continuous Delivery画面から対象GitRepoの「Force Update」を実行してください。

再作成後、Podが `Running` になったら接頭辞を再確認します。

```bash
kubectl -n wordpress exec <新wordpress-pod> -- grep table_prefix /bitnami/wordpress/wp-config.php
```
...
kubectl -n wordpress exec <wordpress-pod> -- mv /bitnami/wordpress/wp-content /bitnami/wordpress/wp-content.orig
kubectl -n wordpress cp wp-content.tar.gz <wordpress-pod>:/tmp/wp-content.tar.gz
kubectl -n wordpress exec <wordpress-pod> -- tar xzf /tmp/wp-content.tar.gz -C /bitnami/wordpress
kubectl -n wordpress exec <wordpress-pod> -- rm /tmp/wp-content.tar.gz
```
...
```bash
kubectl -n wordpress exec <wordpress-pod> -- rm -rf /bitnami/wordpress/wp-content.orig
```

## 5. DBの復元

```bash
kubectl -n wordpress get pods -l app.kubernetes.io/name=mariadb
ROOTPW=$(kubectl -n wordpress get secret wordpress-mariadb-credentials -o jsonpath='{.data.mariadb-root-password}' | base64 -d)

# 初期データを消してクリーンな状態にする
kubectl -n wordpress exec <mariadb-pod> -- bash -c "mysql -u root -p'$ROOTPW' -e \"DROP DATABASE IF EXISTS bitnami_wordpress; CREATE DATABASE bitnami_wordpress CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci; GRANT ALL PRIVILEGES ON bitnami_wordpress.* TO 'bn_wordpress'@'%'; FLUSH PRIVILEGES;\""

# ダンプを転送してインポート
kubectl -n wordpress cp backup.dump <mariadb-pod>:/tmp/backup.dump
kubectl -n wordpress exec <mariadb-pod> -- bash -c "mysql -u root -p'$ROOTPW' bitnami_wordpress < /tmp/backup.dump"

# 確認
kubectl -n wordpress exec <mariadb-pod> -- bash -c "mysql -u root -p'$ROOTPW' bitnami_wordpress -e \"SHOW TABLES;\""

# 後片付け
kubectl -n wordpress exec <mariadb-pod> -- rm /tmp/backup.dump
```
...
```bash
kubectl -n wordpress exec <wordpress-pod> -- wp search-replace \
  'http://旧ドメイン/パス' 'http://新ドメインまたはLB-IP/パス' --all-tables --precise

kubectl -n wordpress exec <wordpress-pod> -- wp rewrite flush --hard
```
...
- **`PASSWORDS ERROR: You must provide your current passwords when upgrading the release`**
  → [manual-wordpress.md](manual-wordpress.md)の`wordpress-mariadb-upgrade-values`
  Secret作成手順を参照してください。
...
- **PVC削除後に`persistentvolumeclaim "..." not found`でPodがスケジュールされない**
  → Fleetの再同期待ち、またはRancher UIから対象GitRepoの「Force Update」を実行してください。
