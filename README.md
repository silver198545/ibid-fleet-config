...
- `docs/manual-harvester-loadbalancer.md`: Harvester Cloud Provider の IPPool 作成と
  Traefik LB 化の手順書（MetalLB は廃止し、Harvester Cloud Provider に一本化）
...
4. [docs/manual-harvester-loadbalancer.md](docs/manual-harvester-loadbalancer.md) の手順で
   Harvester 管理クラスタに IPPool を作成し、Traefik を LoadBalancer 化します。
